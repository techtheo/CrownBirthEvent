...
const fetchEvents = async () => {
  loading.value = true
  try {
    const { data, totalItems: total } = await getEvents(currentPage.value, itemsPerPage)
    events.value = data
...
<template>
  <div class="max-w-6xl mx-auto mt-10">
    ...
    <div v-if="events.length === 0">
      <p class="flex justify-center items-center">No events found.</p>
    </div>
    
    <div v-else class="overflow-x-auto bg-white shadow-md rounded-lg">
      <h2 class="text-3xl font-semibold text-center mb-8">Your Events</h2>
